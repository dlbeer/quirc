...
static void perspective_unmap(const double *c, const struct quirc_point *in, double *u, double *v) {
	double x = in->x;
	double y = in->y;
	double den = -c[0] * c[7] * y + c[1] * c[6] * y + (c[3] * c[7] - c[4] * c[6]) * x + c[0] * c[4] - c[1] * c[3];
...
static uint8_t otsu(const struct quirc *q) {
		...
		// Calculate histogram
		unsigned int *histogram = (unsigned int *) malloc((UINT8_MAX + 1) * sizeof(unsigned int));
		(void) memset(histogram, 0, 1024UL);
		uint8_t *ptr = q->image;
...
static void record_qr_grid(struct quirc *q, int a, int b, int c) {
	...
	if (qr->grid_size > 21) {
		...
		if (qr->align_region >= 0) {
			...
			flood_fill_seed(q, reg->seed.x, reg->seed.y, QUIRC_PIXEL_BLACK, qr->align_region, find_leftmost_to_line,
			                &psd, 0);
		}
	}
